...
public:
  ...
  void * New(size_t sz, int * Space, bool Zero) {
    ...
    if(_cursor == Super::NUM_SLOTS) {
      if(Super::_aliased) {
	// XXX FIXME THIS ONLY HAPPENS IF WE HIT THE END OF THE PAGE HERE, NOT WHEN WE ALLOC THE LAST SLOT ON A PAGE!!! FIXME
  ...
  size_t freeSlots() const {
    ...
    for(int cur = 0; cur < Super::NUM_SLOTS; cur++) {
      if(!Super::testSlot(cur)) ret++;
    }
    return ret;
  }

  virtual void breakpoint() const {
    if(Super::_aliased) abort();
  }
