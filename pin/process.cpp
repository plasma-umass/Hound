...
struct drag {
  ...
  bool operator<(const drag & rhs) const { return val < rhs.val; }
};
...
int main() {
  ...
  set_intersection(realSites.begin(),realSites.end(),plugSites.begin(),
                   plugSites.end(),inserter(plugInter,plugInter.begin()));
  set_intersection(realSites.begin(),realSites.end(),swatSites.begin(),
                   swatSites.end(),inserter(swatInter,swatInter.begin()));
...

  {
    ...
    for(set<uint32_t>::iterator i = plugInter.begin(); i != plugInter.end();
        i++) {
      //npsum += siteCt[*i];
      npsum += realDrag[*i];
    }   
    for(set<uint32_t>::iterator i = swatInter.begin(); i != swatInter.end();
        i++) {
      //nssum += siteCt[*i];
      nssum += realDrag[*i];
    }   
    for(set<uint32_t>::iterator i = realSites.begin(); i != realSites.end();
        i++) {
      //dsum += siteCt[*i];
      dsum += realDrag[*i];
    }
    for(set<uint32_t>::iterator i = plugSites.begin(); i != plugSites.end();
        i++) {
      //psum += siteCt[*i];
      psum += realDrag[*i];
    }
    for(set<uint32_t>::iterator i = swatSites.begin(); i != swatSites.end();
        i++) {
      //ssum += siteCt[*i];
      ssum += swatDrag[*i];
    }
  ...
  */
  for(map<unsigned long, uint64_t>::iterator i = realDrag.begin();
	i != realDrag.end(); i++) {
    realSort.insert(drag(i->second, i->first));
  }
  for(map<unsigned long, uint64_t>::iterator i = swatDrag.begin();
	i != swatDrag.end(); i++) {
    swatSort.insert(drag(i->second, i->first));
  }
  for(map<unsigned long, uint64_t>::iterator i = plugDrag.begin();
	i != plugDrag.end(); i++) {
...
#define LIM 20
  ...
  for(set<drag>::reverse_iterator i = realSort.rbegin(); i != realSort.rend(); i++) {
    cout << std::hex << i->site << '\t' << std::dec << i->val << '\t' << swatDrag[i->site]
	<< '\t' << plugDrag[i->site] << endl;
    if(++ct == LIM) break;
  }
