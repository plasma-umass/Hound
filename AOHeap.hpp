...
public:
  AOHeap(ULONG callsite_hash = 0ul) : 
    _cs_hash(callsite_hash),
    _locked(0)
  {
  }
  ...
  PVOID New(size_t sz, int * Space, bool Zero) {
    ...
    if(_curr[sc] == NULL) allocNewBlock(sc);
  ...
  inline HASH_TYPE getCallSite() const { return _cs_hash; }
  ...
  virtual void removeBlock(Node * bl) {
    ...
    if(_curr[sc] == bl->data) _curr[sc] = NULL;
