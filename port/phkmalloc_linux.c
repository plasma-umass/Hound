...
#define index2ptr(foo) (((u_long)(foo) + malloc_origo) << malloc_pageshift)
...
/* Macro for mmap */
#define MMAP(size) mmap(NULL, (size), PROT_READ | PROT_WRITE, MAP_ANON | MAP_PRIVATE, MMAP_FD, (off_t)0);
  ...
  for (i = 0; i < 3; i++) {
    ...
    for (; p != NULL && *p != '\0'; p++) {
      ...
      case 'Z':
        ...
        break;
      default:
        _malloc_message(_getprogname(), malloc_func, " warning: ", "unknown char in MALLOC_OPTIONS\n");
        break;
      }
    }
  }
...
static __inline int malloc_make_chunks(int bits) {
  struct pginfo *bp;
  void *pp;
  int i, k, l;
...
static __inline void free_bytes(void *ptr, u_long index, struct pginfo *info) {
  int i;
  struct pginfo **mp;
  void *vp;
...
void *phkmalloc(size_t size) /* <eric> */
{
  return (pubrealloc(NULL, size, " in malloc():"));
}

void phkfree(void *ptr) /* <eric> */
{
  pubrealloc(ptr, 0, " in free():");
}

void *phkrealloc(void *ptr, size_t size) /* <eric> */
{
  return (pubrealloc(ptr, size, " in realloc():"));
}

char phk_tryfree(void *ptr) {
  ...
  if (info == MALLOC_NOT_MINE)
    return 0;

  pubrealloc(ptr, 0, " plugfree():");
  return 1;
}

size_t phkmalloc_usable_size(void *ptr) { /* <eric> */
...
void *phkmalloc_normalize(void *ptr) {
