...
public:
  AOBlock() : PageBlock(), _aliased(false) {
  }
  ...
  virtual unsigned int getSizeClass() const {
    return LOG_OBJECT_SIZE - LOG_MIN_ALLOC;
  }

  virtual size_t getAllocs() const {
    return AOMergeable<N>::_pop;
  }
...
protected:
  virtual ~AOBlock() {
    ...
    if (_aliased) {
      ...
      // someone could map this out from under us here if we race.  but in that case the memory has still been recycled,
      // so we're ok
      if ((void *)MAP_FAILED ==
          mmap(_start, AO_BLOCK_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED | MAP_ANONYMOUS, -1, 0)) {
